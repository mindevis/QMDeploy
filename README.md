...
2. **Одна команда от root** — K3s, Helm, секреты **`qm-mysql` + `qm-app`** (если задан ключ), Argo CD и Application **qm**:
   ...
   Опции лицензии (те же, что у **`k8s-manage.py secrets`**): **`--cloud-license-ips`**, **`--cloud-license-machine-ids`**, **`--cloud-license-node-names`**; пересоздать секреты: **`--recreate-secrets`**; **`--cloud-license-key '…'`** вместо файла (после выполнения см. очистку history в справке **`k8s-manage.py secrets --help`**). Прочие флаги: **`--argocd-host`**, **`--qm-repo-url`**, **`--qm-repo-revision`**, **`--qm-namespace`**, **`--argocd-skip-qm-app`**, **`--skip-argocd`** (только K3s+Helm без Argo — для ручного сценария). Переменные: **`NAMESPACE`**, **`KUBECONFIG`**, **`SKIP_SECRET_CHECK=1`**, **`ARGOCD_HOST`**.
