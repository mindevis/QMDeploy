...
2. **Секреты** в namespace **`qm`** до первого **Sync** Argo (иначе QMServer Cloud, QMSecret и MySQL не стартуют корректно):
   ...
   **GHCR:** в **`values-argocd.yaml`** задан **`imagePullSecrets: ghcr-credentials`** — создайте секрет в **`qm`** (`kubectl create secret docker-registry ghcr-credentials …`), иначе поды не смогут скачать образы.
