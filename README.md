...
- **Legacy:** прямой **`helm upgrade --install`**: **`sudo python3 scripts/install-k3s-helm.py --direct-helm -f my-values.yaml`** (нужен готовый **Helm 3**, если не ставили через скрипт).
...
3. Установка K3s (если ещё нет), Helm, **Argo CD** и автоматическая регистрация Application **qm** (синк из Git, **`values-argocd.yaml`**):
   ...
   sudo python3 scripts/install-k3s-helm.py
   ```
...
sudo python3 /opt/qm/scripts/install-k3s-helm.py
```
