...
2. **Секреты** в namespace **`qm`** до первого **Sync** Argo (иначе QMServer Cloud, QMSecret и MySQL не стартуют корректно):
   ...
   export QMSERVER_CLOUD_LICENSE_KEY='ваш-ключ-Cloud'
   python3 scripts/create-greenfield-secrets.py -n qm
   ```

   Скрипт создаёт **`qm-mysql`** и **`qm-app`**: DSN, **`JWT_SECRET`**, **`QMSERVER_CLOUD_LICENSE_KEY`**, автоматически генерирует **`QMSECRET_MASTER_KEY`** и **`QMSECRET_SERVICE_TOKEN`** (нужны для **QMSecret**, включён в **`values-argocd.yaml`**). Опционально привязка лицензии: **`--cloud-license-ips`**, **`--cloud-license-machine-ids`**, **`--cloud-license-node-names`** (или соответствующие env).
