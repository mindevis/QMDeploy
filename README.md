...
2. **Секреты** в namespace **`qm`** до первого `helm upgrade --install` (иначе MySQL и приложения не стартуют):
...
3. Установка K3s (если ещё нет) и чарта:
   ...
   Переменные: **`NAMESPACE`**, **`RELEASE_NAME`**, **`KUBECONFIG`**, опционально **`SKIP_SECRET_CHECK=1`** (не подсказывать про `qm-mysql`).

4. **Полная переустановка** с нуля (потеря данных MySQL и загрузок): удалите релиз и PVC, затем снова пункты 2–3:

   ```bash
   helm uninstall qm -n qm
   kubectl delete pvc -n qm --all
   ```

   При необходимости: `kubectl delete namespace qm`, создайте namespace снова через скрипт секретов.
