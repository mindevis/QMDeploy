...
2. **Секреты** в namespace **`qm`** до первого **Sync** Argo (иначе QMServer Cloud, QMSecret и MySQL не стартуют корректно):
   ...
   ```bash
   cd QMDeploy
   # от root на сервере K3s; ключ только через аргументы (предпросмотр: добавьте --dry-run)
   python3 scripts/create-greenfield-secrets.py -n qm --cloud-license-key 'ваш-ключ-Cloud'
   ```

   Скрипт создаёт **`qm-mysql`** и **`qm-app`**: DSN, **`JWT_SECRET`**, **`QMSERVER_CLOUD_LICENSE_KEY`**, автоматически генерирует **`QMSECRET_MASTER_KEY`** и **`QMSECRET_SERVICE_TOKEN`** (нужны для **QMSecret**, включён в **`values-argocd.yaml`**). Опционально привязка лицензии: **`--cloud-license-ips`**, **`--cloud-license-machine-ids`**, **`--cloud-license-node-names`**. **`--dry-run`** — сводка и маскированные значения, без **`kubectl create`**.
...
Если секретов ещё нет — сначала **`create-greenfield-secrets.py`** с **`--cloud-license-key`** (см. раздел выше).
