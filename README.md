...
- **QMServer в K8s — только редакция Cloud:** чарт выставляет **`QMSERVER_CLOUD_K8S=1`** и тянет образ **`ghcr.io/<owner>/qmserver`** (Cloud). Нужен **лицензионный ключ** в Secret **`qm-app`** (**`QMSERVER_CLOUD_LICENSE_KEY`**); см. **`create-greenfield-secrets.py`**.
...
2. **Совсем пустой сервер (нет `kubectl` / K3s)** — сначала только кластер и Helm, без Argo:
   ...
   Дальше пункты 3–5. **`create-greenfield-secrets.py`** использует **`kubectl`** или **`k3s kubectl`** и при необходимости **`KUBECONFIG=/etc/rancher/k3s/k3s.yaml`**.
3. **Секреты** в namespace **`qm`** до первого **Sync** Argo (иначе QMServer Cloud, QMSecret и MySQL не стартуют корректно):

   ```bash
   cd QMDeploy
   # от root на сервере K3s; предпочтительно ключ из файла (не светится в ps / history так же, как в argv):
   install -m 600 /dev/null /root/.qm-cloud-license
   nano /root/.qm-cloud-license   # один раз вставить ключ, сохранить
   python3 scripts/create-greenfield-secrets.py -n qm --cloud-license-key-file /root/.qm-cloud-license
   # либо --cloud-license-key '…' — после выхода скрипт по возможности удалит эту строку из ~/.bash_history / $HISTFILE; в текущей сессии bash: history -d -1
   ```

   Скрипт создаёт **`qm-mysql`** и **`qm-app`**: DSN, **`JWT_SECRET`**, **`QMSERVER_CLOUD_LICENSE_KEY`**, автоматически генерирует **`QMSECRET_MASTER_KEY`** и **`QMSECRET_SERVICE_TOKEN`**. Опционально привязка лицензии: **`--cloud-license-ips`**, **`--cloud-license-machine-ids`**, **`--cloud-license-node-names`**. **`--dry-run`** — без **`kubectl`**. Отключить правку файлов истории при **`--cloud-license-key`**: **`--no-scrub-history`** или **`QM_NO_SCRUB_HISTORY=1`**.

   **GHCR:** в **`values-argocd.yaml`** задан **`imagePullSecrets: ghcr-credentials`** — создайте секрет в **`qm`** (после пункта 3; `kubectl` или `k3s kubectl …`), иначе поды не смогут скачать образы.

   Повтор без **`--force`**: ошибка «уже существует». **`--force`** пересоздаёт секреты (при живой БД смените пароли вручную). Доп. ключи Cloud: **`kubectl edit secret qm-app -n qm`**.

4. **Установка Argo CD** и Application **qm** (если пункт 2 уже делали — K3s есть; скрипт пропустит повторную установку K3s/Helm при необходимости):
...
5. **Полная переустановка** с нуля (потеря данных MySQL и загрузок): при **GitOps** сначала удалите Application **qm** в **argocd** (cascade/prune в Argo удалит ресурсы релиза **qm**). Либо вручную: `kubectl delete application qm -n argocd`, затем при «залипшем» релизе **`helm uninstall qm -n qm`**. Далее PVC и namespace:
...
Если секретов ещё нет — сначала **`create-greenfield-secrets.py`** с **`--cloud-license-key-file`** или **`--cloud-license-key`** (см. раздел выше).
