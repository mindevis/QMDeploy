...
- **QMServer в K8s — только редакция Cloud:** чарт выставляет **`QMSERVER_CLOUD_K8S=1`** и тянет образ **`ghcr.io/<owner>/qmserver`** (Cloud). Ключ в **`qm-app`**: передайте **`--cloud-license-key-file`** или **`--cloud-license-key`** в **`install-k3s-helm.py`**, либо отдельно **`create-greenfield-secrets.py`**.
- **Режим по умолчанию:** **`install-k3s-helm.py`** ставит **K3s** и **Helm** при необходимости, опционально создаёт **greenfield-secrets**, затем **Argo CD** и Application **«qm»** (GitOps, **`values-argocd.yaml`**).
...
2. **Одна команда от root** — K3s, Helm, секреты **`qm-mysql` + `qm-app`** (если задан ключ), Argo CD и Application **qm**:

   ```bash
   cd QMDeploy
   install -m 600 /dev/null /root/.qm-cloud-license && nano /root/.qm-cloud-license   # вставить лицензию Cloud
   python3 scripts/install-k3s-helm.py --cloud-license-key-file /root/.qm-cloud-license
   ```

   Опции лицензии (как у **`create-greenfield-secrets.py`**): **`--cloud-license-ips`**, **`--cloud-license-machine-ids`**, **`--cloud-license-node-names`**; пересоздать секреты: **`--recreate-secrets`**; **`--cloud-license-key '…'`** вместо файла (после выполнения см. очистку history в справке **`create-greenfield-secrets.py`**). Прочие флаги: **`--argocd-host`**, **`--qm-repo-url`**, **`--qm-repo-revision`**, **`--qm-namespace`**, **`--argocd-skip-qm-app`**, **`--skip-argocd`** (только K3s+Helm без Argo — для ручного сценария). Переменные: **`NAMESPACE`**, **`KUBECONFIG`**, **`SKIP_SECRET_CHECK=1`**, **`ARGOCD_HOST`**.

   Отдельно только секреты / **`--dry-run`**: **`scripts/create-greenfield-secrets.py`** (см. **`--help`**).

3. **GHCR:** сразу после шага 2 (пока есть **`kubectl` / `k3s kubectl`**) создайте **`ghcr-credentials`** в **`qm`**, иначе образы не скачаются — при необходимости **Sync** в Argo ещё раз.

4. **Мониторинг (Grafana):** в **`values-argocd.yaml`** по умолчанию **`monitoring.enabled: false`**. Включение — в Git или Argo, **`GRAFANA_ADMIN_PASSWORD`** в **`qm-app`** при необходимости.
...
cd QMDeploy
python3 scripts/install-k3s-helm.py --cloud-license-key-file /root/.qm-cloud-license
# или секреты уже в кластере: python3 scripts/install-k3s-helm.py
```
