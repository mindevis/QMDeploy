...
6. **Полная переустановка** с нуля (потеря данных MySQL и загрузок): при **GitOps** сначала удалите Application **qm** в **argocd** (cascade/prune в Argo удалит ресурсы релиза **qm**). Либо вручную: `kubectl delete application qm -n argocd`, затем при «залипшем» релизе **`helm uninstall qm -n qm`**. Далее PVC и namespace:
