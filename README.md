...
**Сервер K3s:** все шаги от **root**; основной вход — **`scripts/k8s-manage.py`** (подкоманды **bootstrap** / **secrets** / **addons**). Старые скрипты **`install-k3s-helm.py`** и т.д. сохранены. **`sync-from-github.py`** для кэша **`/opt/qm`**.
...
- **Legacy-прямой Helm:** **`python3 scripts/k8s-manage.py bootstrap --direct-helm -f my-values.yaml`** (или **`install-k3s-helm.py`**).
...
cd QMDeploy
python3 scripts/k8s-manage.py --cloud-license-key-file /root/.qm-cloud-license
# или секреты уже в кластере: python3 scripts/k8s-manage.py
```
